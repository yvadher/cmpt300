...
void test_process_ancestors(void);
...
void test_process_ancestors(){
	...
	long result = syscall(PROCESS_ANCESTORS_SYS_CALL, array, ARRAY_SIZE, &num_filled);
	...
	// should atleast have swapper, init, and the process itself 
	assert(num_filled >= 3);
	...
	printf("\nProcess ancestots SYS_CALL PASSED all test cases \n");
}